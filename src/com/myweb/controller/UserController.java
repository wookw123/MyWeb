...
public class UserController extends HttpServlet {
	...
	protected void dispatchServlet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/user/join.user")) {	//회원가입 화면 처리
		...
		} else if(command.equals("/user/mypage.user")) { //마이페이지 화면요청
		...
		} else if(command.equals("/user/update.user")) { //수정화면 이동요청
			 ...
			 * 2. execute메서드에서는 update() 메서드를 실행시키고 성공실패 여부를 받아서 컨트롤러로 리턴
			 * 
			 */
			 ...
			 * 3. 성공실패 결과를 컨트롤러로 받아와서 성공시엔, 세션삭제후 홈화면 으로 리다이렉트
			 * 4. 실패시에는 실패메시지를 delete.jsp로 처리해주세요.
			 * 
			 */
